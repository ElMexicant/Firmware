...
/*	 if (!_lastArmedState && armedState) {
	...
	if (beaconPositionUpdated) {
		if (_sensorTimeout & SENSOR_BEACON) {
			beaconInit();
...
void BlockLocalPositionEstimator::publishLocalPos()
{
	...
	if (PX4_ISFINITE(_x(X_x)) && PX4_ISFINITE(_x(X_y)) && PX4_ISFINITE(_x(X_z)) &&
		...
		if (_sub_beacon_position.get().rel_pos_valid) {
		_pub_lpos.get().x = -_sub_beacon_position.get().x_rel;  // north
		_pub_lpos.get().y = _sub_beacon_position.get().y_rel; // east
		} /*else {
		_pub_lpos.get().x = xLP(X_x); 	// north
		_pub_lpos.get().y = xLP(X_y);  	// east
		}*/

		if (_sub_lidar != nullptr && _sub_lidar->get().current_distance < 20) { //rickyremove maybe?
			_pub_lpos.get().z = _sub_lidar->get().current_distance;
		} else if (_fusion.get() & FUSE_PUB_AGL_Z) {
		...
		_pub_lpos.get().vx = _sub_beacon_position.get().vx_rel; // north
		_pub_lpos.get().vy = -_sub_beacon_position.get().vy_rel; // east
		}*/
		_pub_lpos.get().vx = xLP(X_vx); // north
		_pub_lpos.get().vy = xLP(X_vy); // east
		_pub_lpos.get().vz = _sub_lidar->get().dv; //xLP(X_vz); // down //rickyremove
		...
		_pub_lpos.get().dist_bottom = _sub_lidar->get().current_distance;
		_pub_lpos.get().dist_bottom_rate = _sub_lidar->get().dv;
...
void BlockLocalPositionEstimator::predict()
{
	...
	_P += dP;
	_xLowPass.update(_x);
	if(_sub_lidar != nullptr){
	_aglLowPass.update(_sub_lidar->get().current_distance);
	} else {
